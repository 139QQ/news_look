...
class EastMoneySimpleCrawler(BaseCrawler):
    ...
    def crawl(self, category=None, max_news=5, days=1):
        """
        爬取东方财富网新闻
                    ...
                    if news:
                        try:
                            ...
                            if pub_time >= date_limit:
                                ...
                                if self.db_manager:
                                    try:
                                        self.db_manager.insert_news(news)
                                        logger.info(f"已将新闻保存到数据库: {news['title']}")
                                    except Exception as e:
                                        logger.warning(f"保存新闻到数据库失败: {str(e)}")
                            else:
...
def main():
    ...
    # 爬取新闻
    news_list = crawler.crawl(category='财经', max_news=3, days=1)
